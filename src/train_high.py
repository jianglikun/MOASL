...
from utils import query_moa_function,query_moa_high
...
def train_all(data_dir,fold,batch_size,num_epoch,device,logname,embedding_size):
    ...
    for epoch in range(1, num_epoch):
        ...
        save_df = query_moa_high(test_embeddings, train_embeddings, test_labels, train_labels,label_dict)
        if find_num > is_high:
            is_high = find_num
            save_df.to_csv('moa_high_{}.csv'.format(fold))
...
def train_single_cell(data_dir,fold,batch_size,num_epoch,device,cell,logname):
    ...
    for epoch in range(1, num_epoch):
        ...
        for batch_idx, (data, labels) in enumerate(train_loader):
            ...
            if batch_idx % 20 == 0:
                print(
                    "Fold {} Epoch {} Iteration {}: Loss = {}, Number of mined triplets = {}".format(
                        fold,epoch, batch_idx, loss, mining_func.num_triplets))
        train_embeddings, train_labels = get_all_embeddings(train_dataset, model)
        test_embeddings, test_labels = get_all_embeddings(test_dataset, model)
        ...
        test_labels = test_labels.squeeze(1)
        sample_num,find_num = query_moa_function(test_embeddings, train_embeddings, test_labels, train_labels)
        query_moa_high(test_embeddings, train_embeddings, test_labels, train_labels,label_dict)
    ...
    test_log_df['sample_num'] = test_num
    test_log_df['find'] = test_find
    test_log_df.to_csv('result/{}_fold{}_{}_high.csv'.format(cell,fold,logname))
