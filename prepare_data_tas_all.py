...
def save_label_dict(data_info_dir):
    ...
    save_file = '{}/sig2drugmoa.npz'.format(data_info_dir)
    np.savez(save_file, **sig2drugmoa_dict)
    save_file = '{}/drug2moa.npz'.format(data_info_dir)
    np.savez(save_file, **drug2moa_dict)

    return moa_df,level5_info_df,sig2drugmoa_dict,drug2moa_dict

def get_all_sigdata(sig_info_df,data_all_dir):
    ...
    save_file = '{}/all_signature_{}.h5'.format(data_all_dir,sig_data_df.shape[0])
    sig_data_df.to_hdf(save_file,key = 'dat')

def all_data_kfold(moa_df,sig_info_df,data_all_dir):
    # load all data
    all_data_file = '{}/all_signature_62374.h5'.format(data_all_dir)
    ...
    for k,(train_id,test_id) in enumerate(kfold_data):
        ...
        train_save_file = '{}/Train_fold_{}.h5'.format(data_all_dir,k)
        test_save_file = '{}/Test_fold_{}.h5'.format(data_all_dir,k)
...
def cell_train_test(data_cell_dir,data_all_dir,moa_df,sig_info_df):
    ...
    for k, (train_id, test_id) in enumerate(kfold_data):
        ...
        for cell in cell_lines:
            save_cell_dir = '{}/{}'.format(data_cell_dir, cell)
            if not os.path.exists(save_cell_dir):
                os.mkdir(save_cell_dir)

            pert_time = [6,24]
            sig_info_df = sig_info_df[sig_info_df['pert_time'].isin(pert_time)]

            minsize_dup = 5
            cell_sig_info_df = sig_info_df[sig_info_df['cell_iname']==cell]
            cell_sig_info_drug_counts = cell_sig_info_df['pert_id'].value_counts()
            cell_sig_info_drug_counts = cell_sig_info_drug_counts[cell_sig_info_drug_counts>=minsize_dup]
            cell_sig_info_df = cell_sig_info_df[cell_sig_info_df['pert_id'].isin(cell_sig_info_drug_counts.index)]

            train_info_df = cell_sig_info_df[cell_sig_info_df['pert_id'].isin(train_drugs)]
            test_info_df = cell_sig_info_df[cell_sig_info_df['pert_id'].isin(test_drugs)]

            train_data_df = all_data_df.loc[train_info_df['sig_id']]
            test_data_df = all_data_df.loc[test_info_df['sig_id']]

            train_save_file = '{}/{}/Train_fold_{}.h5'.format(data_cell_dir,cell, k)
            test_save_file = '{}/{}/Test_fold_{}.h5'.format(data_cell_dir,cell, k)

            train_data_df.to_hdf(train_save_file, key='dat')
            test_data_df.to_hdf(test_save_file, key='dat')
            print('### {} Fold {} saved, train & test drugs is:'.format(cell,k), len(set(train_info_df['pert_id'])),
                  len(set(test_info_df['pert_id'])))
            print('### {} Fold {} saved, train & test signature is:'.format(cell,k), train_data_df.shape, test_data_df.shape)
...
if __name__ == '__main__':
    ...
    # get_all_sigdata(level5_info_df,data_all_dir)

    # make all data cross cell line, Train and Test
    all_data_kfold(moa_df,level5_info_df,data_all_dir)
