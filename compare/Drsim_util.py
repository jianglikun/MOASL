...
def sigidTo(MOA):
    ...
    label_file = '/mnt/data1/jlk/Cmap_database/DrSim/Drsim_original/{}SigInfo.tsv'.format(MOA)
...
### get drug MOA class
def drugTOMOA():
    label_file = '/mnt/data1/jlk/Cmap_database/DrSim/Drsim_original/MOASigInfo.tsv'
    drug2MOA = {}
    with open(label_file, 'r') as fin:
        fin.readline()
        for line in fin:
            lines = line.strip().split('\t')
            drug2MOA[lines[2]] = lines[-2]
    return drug2MOA
